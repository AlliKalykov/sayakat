...
class IsCassir(ReadOnly):
    def has_permission(self, request, view):
        return super().has_permission(request, view) or bool(request.user and request.user.role == "cassir")


class IsDispetcher(ReadOnly):
    def has_permission(self, request, view):
        return super().has_permission(request, view) or bool(request.user and request.user.role == "dispetcher")
